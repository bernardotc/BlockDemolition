...
public class BasicPhysicsEngineUsingBox2D {
	...
	public static enum LayoutMode {CONVEX_ARENA, CONCAVE_ARENA, CONVEX_ARENA_WITH_CURVE, PINBALL_ARENA, RECTANGLE, SNOOKER_TABLE, BLOCK_DEMOLITION};
	public BasicPhysicsEngineUsingBox2D() {
		...
		LayoutMode layout=LayoutMode.BLOCK_DEMOLITION;
		// pinball:
...
//			public BasicRectangle(double sx, double sy, double vx, double vy, double width, double height, double orientation, double angularVeloctiy, boolean improvedEuler, Color col, double mass) {
		...
		polygons.add(new BasicPolygon(WORLD_WIDTH/2-2,WORLD_HEIGHT/2+1.3f,1.2f*s,1.2f*s, r*2,Color.YELLOW, 1, rollingFriction,4));*/
                
                polygons.add(new BasicPolygon(WORLD_WIDTH/16 * 10, WORLD_HEIGHT / 3 * 1.10f,0,0, r,Color.YELLOW, 1, rollingFriction, 4));
                polygons.add(new BasicPolygon(WORLD_WIDTH/16 * 10, WORLD_HEIGHT / 3 * 1.31f,0,0, r,Color.YELLOW, 1, rollingFriction, 4));
                polygons.add(new BasicPolygon(WORLD_WIDTH/16 * 10.7f, WORLD_HEIGHT / 3 * 1.10f,0,0, r,Color.YELLOW, 1, rollingFriction, 4));
                polygons.add(new BasicPolygon(WORLD_WIDTH/16 * 10.7f, WORLD_HEIGHT / 3 * 1.31f,0,0, r,Color.YELLOW, 1, rollingFriction, 4));
                polygons.add(new BasicPolygon(WORLD_WIDTH/16 * 10.7f, WORLD_HEIGHT / 3 * 1.51f,0,0, r,Color.YELLOW, 1, rollingFriction, 4));
                polygons.add(new BasicPolygon(WORLD_WIDTH/16 * 11.4f, WORLD_HEIGHT / 3 * 1.10f,0,0, r,Color.YELLOW, 1, rollingFriction, 4));
                polygons.add(new BasicPolygon(WORLD_WIDTH/16 * 11.4f, WORLD_HEIGHT / 3 * 1.31f,0,0, r,Color.YELLOW, 1, rollingFriction, 4));
                polygons.add(new BasicPolygon(WORLD_WIDTH/16 * 11.4f, WORLD_HEIGHT / 3 * 1.51f,0,0, r,Color.YELLOW, 1, rollingFriction, 4));
                polygons.add(new BasicPolygon(WORLD_WIDTH/16 * 12.1f, WORLD_HEIGHT / 3 * 1.10f,0,0, r,Color.YELLOW, 1, rollingFriction, 4));
                polygons.add(new BasicPolygon(WORLD_WIDTH/16 * 12.1f, WORLD_HEIGHT / 3 * 1.31f,0,0, r,Color.YELLOW, 1, rollingFriction, 4));
...
//				particles.add(new ParticleAttachedToMousePointer(WORLD_WIDTH/2,WORLD_HEIGHT/2,0,0, r/2, true, 10000));				
		...
		switch (layout) {
			...
			case SNOOKER_TABLE: {
				...
				barriers.add(new AnchoredBarrier_StraightLine(WORLD_WIDTH, WORLD_HEIGHT, 0, WORLD_HEIGHT, Color.WHITE));
				barriers.add(new AnchoredBarrier_StraightLine(0, WORLD_HEIGHT, 0, 0, Color.WHITE));
                                barriers.add(new AnchoredBarrier_StraightLine(WORLD_WIDTH / 16 * 9, WORLD_HEIGHT / 3, WORLD_WIDTH / 16 * 13, WORLD_HEIGHT / 3, Color.WHITE));
				break;
			}
		}
...
//		particles.add(new BasicParticle(0,0,4,10, r,false, Color.RED, includeInbuiltCollisionDetection, 2));
	...
	public static void main(String[] args) throws Exception {
		...
		JEasyFrame frame = new JEasyFrame(view, "Block Demolition Game");
	...
	public void update() {
		...
		for (BasicPolygon p:polygons) {
			...
			p.notificationOfNewTimestep();
                ...
                for (BasicPolygon p:polygons) {
                    if (p.body.getPosition().y >= WORLD_HEIGHT / 3) {
                        allUnder = false;
                        break;
                    }
                }
...
//	public Body findBodyAtWorldCoords(Vec2 worldCoords) {
//		Vec2 d = new Vec2(0.001f,0.001f);
//        AABB aabb = new AABB(worldCoords.sub(d), worldCoords.add(d));
//		Body b=null;
// 
...
//				arg0.m_shape.testPoint(arg0, arg1)
//				return false;
//			}
//        	
//        };
...
//            Body shapeBody = shapes[j].b; 
//            if(shapeBody.isStatic() == false && 
//                shapes[j].testPoint(shapeBody.getXForm(), worldCoords)  ) { 
